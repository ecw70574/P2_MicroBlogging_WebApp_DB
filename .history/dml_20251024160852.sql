...
-- purpose: check if a username exists, so that we can then get the stored password to login get the user's stored password for login verification
...
-- purpose: register a new user by adding their username, encrypted password, first name, and last name into the database
-- context: in UserService.java 
